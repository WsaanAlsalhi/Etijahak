"""
network_engine.py
يقترح باحثين/موجّهين/متعاونين بناءً على تقاطع اهتماماتهم مع المستخدم،
مع شرح "لماذا هذا التواصل منطقي".
"""
...
def suggest_connections(capability_scores: Dict[str, int], top_n: int = 4) -> List[Connection]:
    ...
    for person in people:
        ...
        reason = f"مشاريعك/مهاراتك في ({overlap_ar}) تتقاطع مع مجال عمل هذا الشخص، مما يجعل التواصل معه منطقيًا."

        results.append(Connection(
            id=person["id"], name_ar=person["name_ar"], role_ar=person["role_ar"],
            type=person["type"], icon=person["icon"], reason_ar=reason, match_score=match_score,
        ))
