...
يطابق قدرات المستخدم مع الفرص المتاحة، بمخرجات ثنائية اللغة.
"""
...
def match_opportunities(capability_scores: Dict[str, int], top_n: int = 5) -> List[Opportunity]:
    ...
    for op in opportunities:
        ...
        if matched_skills:
            skills_ar = "، ".join(s.replace("_", " ") for s in matched_skills[:3])
            skills_en = ", ".join(s.replace("_", " ").title() for s in matched_skills[:3])
            reason_ar = f"تطابق قوي في: {skills_ar}"
            reason_en = f"Strong match in: {skills_en}"
        else:
            reason_ar = "يحتاج بناء المهارات الأساسية المطلوبة لهذه الفرصة"
            reason_en = "Needs to build the core skills required for this opportunity"

        results.append(Opportunity(
            id=op["id"], name_ar=op["name_ar"], name_en=op.get("name_en", op["name_ar"]),
            type=op["type"], icon=op["icon"],
            match_score=match_pct, reason_ar=reason_ar, reason_en=reason_en,
        ))
