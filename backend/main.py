...
def analyze(
    ...
    # الهدف إما ثابت من goals.json، أو مخصص ولّده الذكاء الاصطناعي (مفتاحه يبدأ بـ custom:)
    if payload.goal_key.startswith("custom:"):
        try:
            custom_id = int(payload.goal_key.split(":", 1)[1])
